...
const PlantCard: React.FC<PlantCardProps> = ({ card }) => {
  ...
  //could probably create a memo to use hydration status
  //and could probably also use a memo to check percentage
    const percentage = (card.waterAdded/card.waterNeeded) * 100 || 1
    ...
    <div className="bg-yellow-200 p-4 rounded-lg shadow-md">
      ...
      <p className="text-gray-700">Water Added: {card.waterAdded} / {card.waterNeeded}</p>
      <div className="h-4 bg-gray-200 rounded">
      <div
        className={`h-full rounded transition-width duration-500 animate-pulse ${status === "I'm thirsty!" ? "bg-red-400" : "bg-blue-300"}`}
        style={{ width: `${percentage}%` }}
      ></div>
      <button disabled={status === "I'm thirsty!" ? false : true} onClick={() => {
